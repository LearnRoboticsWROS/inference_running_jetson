...
def qos_video(depth: int = 1) -> QoSProfile:
    """
    QoS tuned for video-like streams: best-effort + small queue.
    """
    return QoSProfile(
...
class ThreeDPoseEstimationPublisher(Node):
    """
    YOLO + depth -> 3D position (X,Y,Z) in camera optical frame.

    Publishes:
      1) Annotated image (raw optional + compressed)
      2) geometry_msgs/PoseArray with 3D positions for all detections
    """
    def __init__(self):
        super().__init__("three_d_pose_estimation_publisher")
        ...
        # Output: 3D poses
        # ----------------
        self.declare_parameter("pose_topic", "/detected_objects/poses")
        self.declare_parameter("publish_hold_poses", True)  # include last pose if depth invalid
        ...
        self.publish_hold_poses = bool(self.get_parameter("publish_hold_poses").value)
        ...
        # Publisher (poses)
        self.pub_poses = self.create_publisher(PoseArray, self.pose_topic, self.qos)
        ...
        self.get_logger().info(f"Image:  {self.output_topic} (+/compressed)")
        self.get_logger().info(f"Poses:  {self.pose_topic} (PoseArray)")
    ...
    def cb_color(self, msg: Image):
        ...
        try:
            ...
            # Prepare PoseArray output (one per frame)
            pose_arr = PoseArray()
            pose_arr.header = msg.header
            # Ensure frame_id is the camera optical frame (reference for XYZ)
            pose_arr.header.frame_id = self._cam_frame_id or msg.header.frame_id

            if r0.boxes is None or len(r0.boxes) == 0:
                self.pub_poses.publish(pose_arr)
            ...
            for i, xyxy in enumerate(boxes_xyxy):
                ...
                if names is not None and clss is not None:
                    label = r0.names.get(int(clss[i]), "")
                c = float(confs[i])

                # If no depth, optionally publish HOLD pose
                if Z is None:
                    if self.publish_hold_poses and prev is not None and (now - tr["t"]) <= self.hold_last_secs:
                        X, Y, Zuse = prev
                        pose_arr.poses.append(self._pose_xyz(X, Y, Zuse))
                        cv2.putText(
                            annotated,
                ...
                # Append pose to PoseArray
                pose_arr.poses.append(self._pose_xyz(X, Y, Z))
            ...
            # Publish PoseArray first (so downstream can react ASAP)
            self.pub_poses.publish(pose_arr)
    ...
    @staticmethod
    def _pose_xyz(x: float, y: float, z: float) -> Pose:
        """
        Build a Pose with position (x,y,z) and identity orientation.
        """
        p = Pose()
    ...
    def _publish_image(self, bgr_img: np.ndarray, header):
...
def main():
    rclpy.init()
    node = ThreeDPoseEstimationPublisher()
    try:
        rclpy.spin(node)
